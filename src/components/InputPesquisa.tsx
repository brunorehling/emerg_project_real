...
export function InputPesquisa({ setReviews }: InputPesquisaProps) {
    ...
    return (
        <div className="flex flex-col items-center mt-3 gap-2">
            <form className="w-full max-w-3xl flex" onSubmit={handleSubmit(enviaPesquisa)}>
                <input
                    type="search"
                    placeholder="Informe o livro ou o título da review"
                    {...register("termo")}
                    className="flex-1 p-3 ps-10 rounded-l-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:placeholder-gray-400 transition"
                />
                <button
                    type="submit"
            ...
            {/* Botão Voltar para todas as reviews */}
            <button
                onClick={buscaTodasReviews}
                className="mt-2 px-4 py-2 bg-gray-500 hover:bg-gray-600 text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-300 transition"
            >
                Voltar
            </button>
        </div>
